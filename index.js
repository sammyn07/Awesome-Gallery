const accessKey = "FeiPIVBvUmCXIKA749882n28hZmpCA6wqsaRBn8JpNo";
 ...
 function openFullscreen(imageSrc, imageAlt) {
     ...
     history.pushState({ action: "fullscreen" }, "", window.location.href);
 ...
 function handleSearchHistory(query) {
     ...
     history.pushState({ action: "search", query: query }, '', url.href);
 }
 ...
 window.addEventListener('popstate', (event) => {
     if (isFullscreenActive) {
         ...
         const savedQuery = event.state?.query || "";
         if (savedQuery) {
             searchInputEl.value = savedQuery;
             ...
             currentPage = 1;
             searchResultsEl.innerHTML = "";
             getPreloadedImages(1, preloadedImagesPerPage).then(initialImages => {
                 initialImages.forEach(image => {
                     const card = createImageCard(image);
                     searchResultsEl.appendChild(card);
                 });
